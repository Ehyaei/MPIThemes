...
\definecolor{bannerColor}{HTML}{009EE2}

\definecolor{bannerTextColor}{HTML}{ffffff}
...
\includegraphics[height=0.06\paperheight]{src/icon/MPIBCNNWhite.eps}
};
\node[font=\color{bannerTextColor},anchor=west,
  xshift=1cm,yshift=-0.7cm] at (current page.north west)
  {
  \small{\textbf{ \Large\textbf{Max-Planck Institute} \\}}
  };
\end{tikzpicture}%
}
...
\node[inner sep=0pt] at (page cs:0.672,0.795){\includegraphics[height=0.126\paperheight]{src/icon/MPILogoGray.eps}};
% add secondary logo % TODO: add else
\node[inner sep=0pt] at (page cs:0.672,-0.795){\includegraphics[height=0.126\paperheight]{src/icon/MPIBCWhite.eps}};
...
\node[below,text width=\paperwidth,font=\color{titlepageTextColor}] at (page cs:0.0+\titleHjust,0.0+\titleVjust) {
  ...
  % add bodoni font to date
    \huge{\uppercase{05 Oktober, 2021}}
  \vspace{0.3cm}
...
\begin{center}
\includegraphics[width=4cm]{src/icon/MPIBCNNBlack.eps}
\end{center}
